...
IF EXISTS(SELECT 1
          ...
          WHERE  ROUTINE_NAME = 'MaintDbPartition'
                 AND SPECIFIC_SCHEMA = 'dbo')
  BEGIN
      DROP PROCEDURE MaintDbPartition
  END
GO
USE msdb
GO
-- EXECUTE msdb.[dbo].MaintDbPartition @PartitionDBName = 'MyPartitions', @PartitionDBEnv = 'JULIE16', @DBFileCd = 'MP'
...
--print @MaxQFileYear
DECLARE @FileSizeMB DECIMAL(10,0)
DECLARE @FileGrowthMB DECIMAL(10,0)
DECLARE @MaxFileSize char(9)
SELECT @FileSizeMB =  CONVERT(DECIMAL(10,0),size/128.0), @FileGrowthMB =  CONVERT(DECIMAL(10,0),growth/128.0), @MaxFileSize = 'UNLIMITED' from sys.master_files
	WHERE DB_NAME(database_id) = @FullDBName and name like '%_' + @FileYearFormat + '_Q'+ @MaxQFileYear
DECLARE @FileSizeMBC varchar(10)
DECLARE @FileGrowthMBC varchar(10)
SELECT @FileSizeMBC = cast(@FileSizeMB AS Varchar(10))
SELECT @FileGrowthMBC = cast(@FileGrowthMB AS Varchar(10))
...
END TRY
BEGIN CATCH
	SELECT @errormessage = 'FileGroup failed to be added. Check the log for the specific error. Manually correct the error and rerun.'
	RAISERROR (@errormessage, 18, 1);
END CATCH
BEGIN TRY
	...
	FILENAME = ''' + @FileName + ''', SIZE = ' + @FileSizeMBC + ', MAXSIZE = ' + @MaxFileSize + ', FILEGROWTH = ' +  @FileGrowthMBC 
	+ ' ) TO FILEGROUP ' + @FileGroupName
	PRINT  @FileGroupFileNameSQL
	EXEC sp_executesql @FileGroupFileNameSQL  
END TRY
BEGIN CATCH
	SELECT @errormessage = 'New FileName failed to be added. Check the log for the specific error, Drop the new FileGroup if it exists. Manually correct the error and rerun.'
	RAISERROR (@errormessage, 18, 1);
END CATCH
...
--PRINT @PartitionFUnctionNameSQL
